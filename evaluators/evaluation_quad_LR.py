...
def quadlr_calibration(DTR, LTR, DTE, LTE, xi):
    def vecxxT(x):
        x = x[:, None]
        xxT = x.dot(x.T).reshape(x.size ** 2, order='F')
        return xxT
...
def evaluation_quad_LR(DTR, LTR, DTE, LTE, L, appendToTitle, PCA_Flag=True):
    for l in L:  # l is a constant, not an array
        ...
        y_01 = numpy.array([])
        for xi in x:
            scores, labels = quadlr_calibration(DTR, LTR, DTE, LTE, xi)
        ...
        plot_DCF(x, y, 'lambda', appendToTitle + 'EVAL_QUADLR_minDCF_comparison')
