...
from classifiers import weighted_logistic_reg_score, tied_cov_GC, logistic_reg_calibration
...
def bayes_error_plot_2best(D, L, pi, title, ylim):
    ...
    pylab.title(title)
    pylab.plot(p, bayes_error_plot(p, D[0], L, minCost=False), color='r', label='SVM_RBF_RAW_actDCF(calibrated)')
    pylab.plot(p, bayes_error_plot(p, D[0], L, minCost=True), 'r--', label='SVM_RBF_RAW_minDCF')

    pylab.plot(p, bayes_error_plot(p, D[1], L, minCost=False), color='b', label='MVG_TIED_RAW_actDCF(calibrated)')
...
def ROC_2best(D, L, pi, title):
    ...
    pylab.plot(FPR, TPR, label="SVM_RBF")
...
def compute_2best_plots(DTR, LTR, DTE, LTE):
    ...
    scores_append = np.hstack(second_scores)
    cal_scores, cal_labels, w, b = calibrate_scores(scores_append, LTE)
    scores_append = scores_append.reshape((1, 4000))
    final_score = np.dot(w.T, scores_append) + b
    #
    ...
    bayes_error_plot_2best([GMM_llrst_raw, final_score], LTE, 0.5, '', 0.4)
    ROC_2best([GMM_llrst_raw, final_score], LTE, 0.5, '')
...
def calibrate_scores(scores, labels):
    ...
    S, estimated_w, estimated_b = logistic_reg_calibration(np.array([scores_70]), labels_70,
                                                           np.array([scores_30]), 1e-3)
